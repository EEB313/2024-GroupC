...
```{r}
world <- list("1" = list("n1" = 10, "n2" = 10), "2" = list("n1" = 0, "n2" = 0), "3" =  list("n1" = 0, "n2" = 0),"4" = list("n1" = 0, "n2" = 0), "5" = list("n1" = 0, "n2" = 0), "6" = list("n1" = 0, "n2" = 0), "7" = list("n1" = 0, "n2" = 0), "8" = list("n1" = 0, "n2" = 0), "9" = list("n1" = 0, "n2" = 0), "10" = list("n1" = 0, "n2" = 0))
params <- list("n1" = list("r" = 1, "k" = 1000), "n2" = list("r" = 10, "k" = 100))
```
...
growth <- function(r, k, n){
  #return((r*n*(1-(n/k)))) #This is the origal base logistic growth equation
  #return((n+(log(r, base = exp(1))*n*(1-(n/k)))))
  return((r*(1-(n/k)))*n)
}
growth(r = 1, k = 1000, n = 10)
```
...
while(i<1000){
  for(patch in seq(1, length(world))){
    ...
    for(pop in seq(1, length(world[[patch]]))){
      ...
      world[[patch]][[curr_pop_name]] <- growth(r = params[[curr_pop_name]][["r"]], k = params[[curr_pop_name]][["k"]], n = curr_pop) -> curr_pop
      print(curr_pop)
    }}
  for(patch in seq(1, length(world))){
    #print(seq(1, length(world)))
  ...
  if(sum(unlist(world$"10")) > 0){
    result <- list(world)
    test_data <- c(test_data, result)
    return(result)    
  }
  i <- i + 1
}

test_frame <- data_frame(worlds = test_data)
test_frame$worlds[1]
```


```{r}
growth_step <- function(world, params){
  for(patch in seq(1, length(world))){
    #print(seq(1, length(world)))
    #print(patch)
    for(pop in seq(1, length(world[[patch]]))){
...
```{r}
migration_step <- function(world, params){
  for(patch in seq(1, length(world))){
    #print(seq(1, length(world)))
    #print(patch)
    for(pop in seq(1, length(world[[patch]]))){
...
```{r}
simulate <- function(world, param){
  while(i < 1000){
    growth_step(world, params) |>
      migration_step(params = param) -> result
    
      if(sum(unlist(result$"10")) > 0){
    result <- world
    return(result)    
  }

...
#test_frame <- tibble(worlds = list(), time = numeric(), parameters = list())

#for(replicate in seq(1, 100, 1)){
i <- 0
while(i<1000){
  for(patch in seq(1, length(world))){
    ...
    for(pop in seq(1, length(world[[patch]]))){
      ...
      world[[patch]][[curr_pop_name]] <- round(curr_pop + growth(r = params[[curr_pop_name]][["r"]], k = params[[curr_pop_name]][["k"]], n = curr_pop)) -> curr_pop
      #print(curr_pop)
    }}
  for(patch in seq(1, length(world))){
    #print(seq(1, length(world)))
  ...
  if(sum(unlist(world$"10")) > 0){
    ...
    simulation_data %>% add_row(worlds = result, timestep = i, parameters = list(params, winner = winner)) -> simulation_data
...
simulation_data <- tibble(worlds = list(), timestep = numeric(), parameters = list(), winner = numeric())

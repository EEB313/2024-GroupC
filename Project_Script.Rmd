...
dummy_world <- vector(mode="list",length=10)
...
```{r plots}
dummy_all %>%
  ggplot(aes(x=timestep, fill=winner)) +
  geom_histogram() +
  facet_wrap(dummy_all$simulation)

dummy_all %>%
  group_by(simulation, winner) %>%
  ggplot(aes(timestep, fill=interaction(simulation,winner))) +
  geom_histogram()
